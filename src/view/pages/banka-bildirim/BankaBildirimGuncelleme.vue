    ...
    <transition name="fade-in-up">
      <div>
        ...
        <v-snackbar v-model="snackbar" absolute bottom right color="success">
          <span>Güncelleme işlemi başarılı şekilde gerçekleşti!</span>
...
export default {
  ...
  methods: {
    validate() {
      ...
      if (this.$refs.form.validate()) {
        ...
        const noti = this.bankNotifications.find(
          p => p.KULLANICIBILDIRIMID === +this.$route.params.id
        );

        noti.KULLANICIBILDIRIMID = +this.$route.params.id;
        noti.SIRKETHESAPID = selectedBank.SIRKETHESAPID;
        noti.ISLEMSECIMTIPI = this.code1.accountIn;
        noti.KARSIADSOYAD = this.code1.name;
        noti.TUTAR = +this.code1.amount;
        noti.EMAIL = this.code1.sendMail ? this.code1.email : "";
        noti.CEPTELEFONU = this.code1.sendSms ? this.code1.phoneNumber : "";
        noti.EXPIRESON = this.code1.expDate;
        noti.OPTIME = this.formatDate(new Date());
    ...
    setFormData() {
      ...
      this.code1.sendMail = noti.EMAIL !== "";
      this.code1.sendSms = noti.CEPTELEFONU !== "";

      this.code1.accountIn = noti.ISLEMSECIMTIPI.toString();
      this.code1.name = noti.KARSIADSOYAD;
      this.code1.amount = noti.TUTAR;
      this.code1.email = noti.EMAIL;
      this.code1.phoneNumber = noti.CEPTELEFONU;
      this.code1.expDate = this.formatDate(noti.EXPIRESON);

      const selectedBank = this.bankTotalAmount.find(
        p => p.SIRKETHESAPID === noti.SIRKETHESAPID
      );

      this.code1.bankId =
        selectedBank.SIRKETADI +
        "-" +
        selectedBank.BANKAADI +
        "-" +
        selectedBank.HESAPNO;
    }
  },
  mounted() {
    ...
    this.setBankData();
    this.setFormData();
    this.$store.dispatch(SET_BREADCRUMB, [{ title: "Bildirim Güncelle" }]);
  },
  computed: {
    ...mapGetters(["currentUser", "bankTotalAmount", "bankNotifications"]),
    bankParams() {
      return {
        params: {
